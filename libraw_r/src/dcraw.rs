...
impl<T> Processor<'_, T> {
    pub fn dcraw_process_make_mem_thumb(&mut self) -> Result<ProcessedImage, LibrawError> {
        let mut errc = 0;
        let data =
            unsafe { sys::libraw_dcraw_make_mem_thumb(self.inner.inner.as_ptr(), &mut errc) };
        assert!(!data.is_null());
    ...
    pub fn dcraw_process(&mut self) -> Result<(), LibrawError> {
        LibrawError::check(unsafe { sys::libraw_dcraw_process(self.inner.inner.as_ptr()) })?;
        Ok(())
    }
    ...
    pub fn dcraw_process_make_mem_image(&mut self) -> Result<ProcessedImage, LibrawError> {
        let mut errc = 0;
        let data =
            unsafe { sys::libraw_dcraw_make_mem_image(self.inner.inner.as_ptr(), &mut errc) };
        assert!(!data.is_null());
        LibrawError::to_result(
            errc,
            ProcessedImage {
    ...
    pub fn dcraw_ppm_tiff_writer(
        ...
        LibrawError::check(unsafe {
            sys::libraw_dcraw_ppm_tiff_writer(
